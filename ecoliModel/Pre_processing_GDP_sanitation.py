...
os.chdir("P:\\11210268-014-flood-and-health\\FloodsAndHealthTool-main")
...
input_dir = r"examples\Data bronnen"
output_dir = r"output\preprocessing"

# Ensure output directory exists
os.makedirs(output_dir, exist_ok=True)

# 📂 Step 2: Define Input File Paths
population_raster = os.path.join(output_dir, "moz_population_combined_compressed.tif")
...
output_raster = os.path.join(output_dir, "ecoli_emissions3.tif")
...
# Crop multiband raster to same extent as flood map
FLOOD_RASTER = r'output\preprocessing\flooded_zoom_new.tif'
emissions_path = r"output\preprocessing\ecoli_emissions3.tif"  # Multi-band raster
output_path = r"output\preprocessing\ecoli_emissions_cropped.tif"  # Output cropped raster
...
# Load and clip emissions raster
with rasterio.open(emissions_path) as emissions_src:
    # Ensure CRS matches
    if emissions_src.crs != flood_crs:
        raise ValueError("⚠️ CRS mismatch! Emissions raster must match flood raster.")

    # Use WarpedVRT to align emissions raster to flood raster
    with WarpedVRT(
        emissions_src,
        crs=flood_crs,
        ...
        # Initialize an empty array to hold all bands
        num_bands = vrt.count
        emissions_cropped = np.empty((num_bands, flood_shape[0], flood_shape[1]), dtype=np.float32)

        # Process each band
        for band_idx in range(1, num_bands + 1):
            emissions_cropped[band_idx - 1] = vrt.read(band_idx, window=emissions_window)
        
        # Get metadata from the emissions raster to use for the output
        kwargs = emissions_src.meta.copy()
        kwargs.update({
            'count': num_bands,  # Ensure we set the correct number of bands
            'dtype': 'float32',  # Set the data type of the output raster
            'height': flood_shape[0],
            'width': flood_shape[1],
            'transform': flood_transform,
            'crs': flood_crs
        ...
        # Write the processed multiband raster
        with rasterio.open(output_path, 'w', **kwargs) as dst:
            dst.write(emissions_cropped)

# Check the result: emissions_cropped is now the clipped and aligned multiband array
print("Cropped emissions raster shape:", emissions_cropped.shape)